...
$(window).keypress(function(e) {
  ...
  console.log(e.which);
    if (!(e.which >= 97 || e.which <= 122) && !(e.which == 228 || e.which == 229 || e.which == 246)) {
      //a-ö will be the only matches, else alert
        return alert("Du har redan valt de bokstaven: " + key + " ... försök en annan liten bokstav");
    }
    ...
    if (/[a-ö]/.test(key)) {
      ...
      if (selectedLetters.hasOwnProperty(key)) {
        return alert("Du har redan valt den, försök en annan");
      }
      else{
        ...
        $('#correct').html('Korrekt: '+shownLetters);
        ...
        if (inWord === false) {
          ...
          $('#chances').html('Chanser kvar: '+gamePieces.length);

          //Show incorrect number of choices
          $('#incorrect').html('Inkorrekt: '+ (10-gamePieces.length));
